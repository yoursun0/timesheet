...
$s_date = "2009-01-01";
$e_date = "2009-03-31";
if(isset($_GET['job_no'])) $job_no = $_GET['job_no'];
if(isset($_GET['dept_id'])) $selected_dept_id = $_GET['dept_id'];
if(isset($_GET['user_id'])) $selected_user_id = $_GET['user_id'];
...
/*	echo "<pre>";
	...
	if (!empty($job_no)) {
		echo "<h3>Job : <font color='blue'>$job_no</font></h3>";
	}
	if (!empty($selected_dept_id)) {
		echo "<h3>Department : <font color='blue'>".$depts[$selected_dept_id]['name']."</font></h3>";
	}
	if (!empty($selected_user_id)) {
		echo "<h3>Staff : <font color='blue'>".$users[$selected_user_id]['name']."</font></h3>";
	}
	echo "<h3>Range : <font color='blue'>$s_date ~ $e_date</font></h3>";
	echo "<hr size=1><table border='1'>";
	echo "<tr>
	<th>Name</th>
	<th>Department</th>
	<th>Start</th>
	...
	while($r = mysql_fetch_array($rs)){
		...
		$dept_name = $depts[$dept_id]['name'];

		$detail_link = "<a href='detail.php?job_no=$job_no&dept_id=$selected_dept_id&user_id=$user_id&s_date=$s_date&e_date=$e_date' target='_blank'>".$user_name."</a>";
	...
	echo "<tr><td colspan='4'>Total Hour : </td><td>$total_hour</td></tr>";
	echo "</table>";
	
	echo "<hr size=1><table border=1><tr align='center'><td>Job</td><td>Total Hour</td>";
	foreach($depts as $dept_id => $dept_detail){
		$detail_link = "<a href='detail.php?job_no=$job_no&dept_id=$dept_id&user_id=&s_date=$s_date&e_date=$e_date' target='_blank'>".$dept_detail['name']."</a>";
	...
	foreach($jobs as $job_id=>$job_details){
		$detail_link = "<a href='detail.php?job_no=".urlencode($job_id)."&dept_id=$selected_dept_id&user_id=$selected_user_id&s_date=$s_date&e_date=$e_date' target='_blank'>$job_id</a>";
