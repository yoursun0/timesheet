...
//------------------------------------------------------------------------------
	...
	$required_fields 	= array('date','name');
	$other_fields 		= array();
...
//------------------------------------------------------------------------------
	if ('CREATE' == $action) {													//create new record
		if (false !== ($insert_id = $db->autoExecuteInsert($GLOBALS['db_table']['holidays'],$data))) {
			CS_Response::success('',array('id'=>$insert_id));
		} else {
			CS_Response::warning($db->getErrorMsg());
		}
	} elseif ('EDIT' == $action) {												//update record
		if (false === $db->autoExecuteUpdate($GLOBALS['db_table']['holidays'],$data,"`id` = '$id'")) {
