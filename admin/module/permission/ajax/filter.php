...
<?php
	...
	$table_columns = array("","room_name");
	
	$count = $db->getOne("SELECT COUNT(*) FROM `{$GLOBALS['db_table']['room']}` WHERE `room_name` LIKE '%".$db->quote($name,false)."%'");
	
	$ts = new TableSorterRequest();
	$limit = $ts->processPagination($pager,$count,15);
	$order = $ts->processSorting($sorter,$table_columns);
	
	$sql = "SELECT * FROM `{$GLOBALS['db_table']['room']}` WHERE `room_name` LIKE '%".$db->quote($name,false)."%' $order $limit";
	$rs = $db->getArray($sql);
