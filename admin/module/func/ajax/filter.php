...
<?php
	...
	$count = $db->getOne("SELECT COUNT(*) FROM `{$GLOBALS['db_table']['func']}` WHERE `name` LIKE '%".$db->quote($name,false)."%'");
	
	$ts = new TableSorterRequest();
	$limit = $ts->processPagination($pager,$count,30);
	$order = $ts->processSorting($sorter,$table_columns);
	
	$sql = "SELECT * FROM `{$GLOBALS['db_table']['func']}` WHERE `name` LIKE '%".$db->quote($name,false)."%' $order $limit";
	$rs = $db->getArray($sql);
