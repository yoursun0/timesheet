...
/*	echo "<pre>";
	...
	echo "<h1>Job : $job_no ($s_date ~ $e_date)</h1>";
	...
	foreach($depts as $dept_id => $dept_detail){
		$detail_link = "<a href='dept_detail.php?job_no=".urlencode($job_no)."&dept_id=$dept_id&s_date=$s_date&e_date=$e_date'  target='_blank'>{$dept_detail['name']}</a>";
	...
	foreach($jobs as $job_id=>$job_details){
		echo "<tr align='center'><td>".$job_id."</td><td>" . $job_details."</td>";
		foreach($depts as $dept_id => $dept_detail){
			echo "<td>";
