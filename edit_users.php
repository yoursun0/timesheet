...
if (isset($Action) && ( ($Action == "Edit") or ($Action == "Add") ))
    ...
    for ($i=0; $i<$nfields; $i++)
        ...
        print "      <td><input type=text name=\"".htmlspecialchars("Field[$i]").
                          "\" value=\"".htmlspecialchars($data[$i])."\" ".(($level == 2) ? "" : "readOnly")."/></td>\n";
...
while ($line = sql_row($list, $i++))
    ...
    $j = -1;
    $this_id = 0;
	/* The 1st data is the ID. */
    $this_id = $line[0];
	/* The 2nd data is the name. */
	$name = $line[1];
	if (getWritable($name, $user)){
		...
		/* Display the name, if any. */
		if ($line[1] == "") $line[1] = "&nbsp;"; // IE doesn't print a frame around void data.
	    print "\t\t<td>".$line[1]."</td>\n";
		
		/* Display the email, if any. */
		if ($line[3] == "") $line[3] = "&nbsp;"; // IE doesn't print a frame around void data.
	    print "\t\t<td>".$line[3]."</td>\n";
