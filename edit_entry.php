...
################################################################################
# added by timmy, 20091104, for access control
################################################################################
	include_once 'include/func.inc.php';
	
	getPermission();
	$approved = getApprovedDate($row[6]);	//6:room_id
	
	if ($approved['time'] > $row[3]) {	//3: start_time
		die('Error: you cannot modify the approved records.');
	}
################################################################################
...
$msecond = ($start_min + ($start_hour * 60)) * 60 + $duration;

$end_hour = intval($msecond / 3600);
$end_minute = ($msecond - ($end_hour * 3600)) / 60;
...
function calHour(){
	if(document.forms["main"].temp_hour.value == "" || document.forms["main"].temp_minute.value == ""){
		alert("Please input time first");
		return false;
	} else {
		var frH = parseInt(document.forms["main"].hour.value, 10);
		var frM = parseInt(document.forms["main"].minute.value, 10);
		var toH = parseInt(document.forms["main"].temp_hour.value, 10);
		var toM = parseInt(document.forms["main"].temp_minute.value, 10);
		var total = (toH - frH) * 60 + (toM - frM);
		if(total <= 0){
			alert("Invalid Time Range");
			return false;
		} else {
			document.forms["main"].duration.value = total;
			document.forms["main"].dur_units.selectedIndex = 0;
			//document.forms["main"].temp_hour.value = "";
			//document.forms["main"].temp_minute.value = ""
			return true;
		}
	}
}
...
{
	document.forms["main"].name.value = jQuery.trim(document.forms["main"].name.value);
	...
	var allowed_code = new Array("AL","SL","OL","ML","WL","PL","NPL","0001","0002","0003","0004","0007","0008","0009","0010");
	var job_code = document.forms["main"].name.value;
	for (k in allowed_code) {
		if (job_code == allowed_code[k]) {
			is_valid_code = true;
			break;
		}
	}
	if (!is_valid_code) {
		...
		// Jxxxx
		if(/^J\d{4}$/.test(job_code)) {
			if (job_code < "J6000") {
				if (!confirm(job_code + " is very old, continue?")) {
					return false;
				}
			}
			is_valid_code = true;
		...
		if(/^C\d{4}$/.test(job_code)) {
			if (job_code > "C1999") {
				alert ("Invalid Range, between C0001~C1999");
				return false;
			}
			is_valid_code = true;
		}
		...
		// 1001 ~ 1999
		if(/^\d{4}$/.test(job_code) && (1001 <= parseInt(job_code, 10) && parseInt(job_code, 10) <= 1999)) {
			is_valid_code = true;
		}
	}


	if (false == is_valid_code) {
  ...
  if (m != 0 && m != 15 && m != 30 && m != 45){
  	alert("Minuate of Start Time must be 00, 15, 30 or 45");
  	return false;
  }
  
  h2 = parseInt(document.forms["main"].temp_hour.value);
  m2 = parseInt(document.forms["main"].temp_minute.value);
  if(h2 > 24 || m2 > 59)
  {
    alert ("<?php echo get_vocab("you_have_not_entered") . '\n' . get_vocab("valid_time_of_day") ?>");
    return false;
  }
  if (m2 != 0 && m2 != 15 && m2 != 30 && m2 != 45){
  	alert("Minuate of End Time must be 00, 15, 30 or 45");
  	return false;
  }
  if (h2 == 24 && m2 > 0){
  	alert("End Time must not larger than 24:00");
  	return false;
  }
  <?php } ?>
...
function checkType(obj) // Auto select the type of the record
{
	t = jQuery.trim(obj.value);
	obj.value = t;
	intExp = /^[jJ][0-9]{4}$/;
	if(t.match(intExp)){
		obj.value = t.toUpperCase();
		$("#type").val("A");
		$("#type_name").html("Job");
	} else if(t == "0001" || t == "0002" || t == "0003"  || t == "0004" || t == "0007" || t == "0008" || t == "0009" || t == "0010"){
		$("#type").val("B");
		$("#type_name").html("0001-0010");
	} else {
		$("#type").val("Z");
		$("#type_name").html("Others");
	}
}
</SCRIPT>
...
<table cellpadding="0" cellspacing="5" border="0"><tr valign="top">
<td>
<TABLE BORDER=0>

<TR><TD CLASS=CR><B><?php echo get_vocab("namebooker")?></B></TD>
  <TD CLASS=CL><INPUT NAME="name" SIZE=40 VALUE="<?php echo htmlspecialchars($name,ENT_NOQUOTES) ?>" onchange="checkType(this)"></TD></TR>
...
to <INPUT NAME="temp_hour" SIZE=2 VALUE="<?=substr("0".$end_hour, -2)?>" MAXLENGTH=2>:<INPUT NAME="temp_minute" SIZE=2 VALUE="<?=substr("0".$end_minute, -2)?>" MAXLENGTH=2>
</TD></TR>
<?php } else { ?>
...
<?php } ?>
<TR style="display: none;"><TD CLASS=CR><B><?php echo get_vocab("duration");?></B></TD>
...
<TR><TD CLASS=CR><B><?php echo get_vocab("type")?></B></TD>
  <TD CLASS=CL><span id="type_name"><?
  $type_array = array();
  $type_array['A'] = "Job";
  $type_array['B'] = "0001-0010";
  $type_array['Z'] = "Others";
  if(array_key_exists($type, $type_array)) echo $type_array[$type];
  ?></span><span style="display: none;"><SELECT NAME="type" ID="type">
<?php
for ($c = "A"; $c <= "Z"; $c++)
{
	if (!empty($typel[$c]))
		echo "<OPTION class=\"$c\" VALUE=$c" . ($type == $c ? " SELECTED" : "") . ">$typel[$c]\n";
}
?></SELECT></span></TD></TR>
...
<td>
<font color="#FF0000"><b>Example of Job No.<b></font>:<br>
J1234 - for Job <font color="#FF0000">(Start with "J" following 4 digits)</font><br>
<br>
AL - for Annual Leave <br>
SL - for Sick Leave <br>
OL - for Other Leave <br>
ML - for Maternity Leave <br>
WL - for Wedding Leave <br>
PL - for Pay Leave <br>
NPL - for Non Pay Leave <br>
<br>
0001 - for General office work / Office meeting / Filing <br>
0002 - for Quotation / Proposal <br>
0003 - for Training <br>
0004 - for Hardware / Programming / Website updating <br>
0007 - for Personnel <br>
0008 - for Accounting <br>
0009 - for ISO <br>
0010 - for Marketing <br>
</td>
</tr>
</table>
