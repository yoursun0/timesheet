...
if (empty($area)){
	if (isset($_SESSION['DefaultAreaID'])) {
		$area = $_SESSION['DefaultAreaID'];
	} else {
		$area = get_default_area();
	}
}
if (empty($room)) {
	if (isset($_SESSION['DefaultRoomID'])) {
		$room = $_SESSION['DefaultRoomID'];
	} else {
...
################################################################################
# added by timmy, 20090911, for access control
################################################################################
include 'include/func.inc.php';

getPermission();
$approved = getApprovedDate($room);
/*echo '<pre>';
print_r($_SESSION);
echo '<pre>';
*/
...
echo "<td width=\"30%\"><u>".get_vocab("rooms")."</u><br>";
  ...
  if ($area_list_format == "select") {
	...
	$sql = "select id, room_name, description from $tbl_room where area_id=$area order by sort_order";
	$res = sql_query($sql);
	if ($res) for ($i = 0; ($row = sql_row($res, $i)); $i++)
	{
		if (!access($row[0], 'view')) {
			continue;
		}
		if ( $pview != 1 )
...
for ($j = 0; $j<=($num_of_days-1) ; $j++) {
	...
	else for ($i = 0; ($row = sql_row($res, $i)); $i++)
	{
 		...
 		if ($row[1] < $am7[$j])
		{
 			...
 			$d[$j][date($format,$am7[$j])]["long_descr"] = 
 			utf8_strftime(hour_min_format(), $start_t)."~".utf8_strftime(hour_min_format(), $end_t+$resolution)."\n"
 			.$row[5];
 			$d[$j][date($format,$am7[$j])]["row_span"] = $row_span_count;
 			
		}
 		else
		{
 			$d[$j][date($format,$start_t)]["data"] = $row[3];
 			$d[$j][date($format,$start_t)]["long_descr"] = 
 			utf8_strftime(hour_min_format(), $start_t)."~".utf8_strftime(hour_min_format(), $end_t+$resolution)."\n"
 			.$row[5];
 			$d[$j][date($format,$start_t)]["row_span"] = $row_span_count;
...
$selectedTime = mktime(12, 0, 0, $month, $day, $year);
$allowedApproveDate = $selectedTime + (8 - date('N', $selectedTime)) * 86400;
//echo date('Y-m-d H:i:s', $allowedApproveDate);
//echo date('Y-m-d H:i:s', $allowedApproveDate - 43200);
?>

<div id="approve_status">
<?php 
if ($selectedTime < $approved['time']) {
	echo <<<HTML
<span style="color:red; font-weight:bold;">Status: Approved & Read Only.</span>
HTML;
} else {
	if (access($room, 'approve') && (time() > $allowedApproveDate)) {
		echo <<<HTML
<a class="txtbutton" href="approve.php?type=week&year=$year&month=$month&day=$day&area=$area&room=$room">Approve This Week</a>
HTML;
	}
}
...
<!--<input type='button' value='Approve Selected Dates' onclick='approveSelected()'>-->
<?php
################################################################################
...
echo '<tr><th width="1%" nowrap>';
//echo '<input type="checkbox" onclick="cbAllApprove(this);" />';
echo "<br><br>".($enable_periods ? get_vocab("period") : get_vocab("time"))."
</th>";
if (empty($dateformat))
	$dformat = "%a<br>%b %d";
else
...
for ($j = 0; $j<=($num_of_days-1) ; $j++)
{
	...
	echo "<th width=\"14%\">",
	//"<input type=\"checkbox\" name=\"approve[]\" value=\"$t\" /><input type='button' value='Approve' onclick='approve($t)'>",
	"<br>
...
)
{
	...
	for ($thisday = 0; $thisday<=($num_of_days-1) ; $thisday++)
	{
		...
		if(!isset($job_summary[$wmonth])) $job_summary[$wmonth]= array();
		...
		$row_span = 1;
 		if(isset($d[$thisday][$time_t]["id"]))
 		{
 			...
 			$row_span = $d[$thisday][$time_t]["row_span"];
 			
 			if(!isset($job_summary[$wmonth][$descr])) $job_summary[$wmonth][$descr]= 0.0;
 			$job_summary[$wmonth][$descr] += intval($row_span);
 		}
 		else
 			unset($id);
...
// Show summary
foreach($job_summary as $m => $j_summary){
	echo "<br><br>".str_repeat("*", 20)." Month $m ".str_repeat("*", 20)."<br>\n";
	...
	echo "<br>";
	asort($j_summary);
	foreach($j_summary as $name => $row_num){
		if($name != ""){
			$hr = ($row_num * 0.25);
			echo $name." = ".$hr."<br>\n";
			if(in_array($name, array("0001", "0002", "0003", "0004", "0007", "0008", "0009", "0010"))){
				$total_hr_2 += $hr;
			} else if(preg_match("/^J[0-9]{4}$/i", $name)){
				$total_hr_1 += $hr;
			} else {
				$total_hr_3 += $hr;
			}
	...
	echo "<br>";
	echo "Job Hours = " . $total_hr_1."<br>Non-Job Hours = ".$total_hr_2."<br>Total Hours = ". ($total_hr_1 + $total_hr_2)."<br><br>Others Hours =".$total_hr_3;
}
