...
for ($i = 0; ($row = sql_row($res, $i)); $i++) {
	...
	for ($t = $start_t; $t <= $end_t; $t += $resolution)
	{
		...
		$today[$row[0]][date($format,$t)]["row_span"]  = 0;
	}
	...
	if ($row[1] < $am7)
	{
		...
		$today[$row[0]][date($format,$am7)]["long_descr"] = 
		utf8_strftime(hour_min_format(), $start_t)."~".utf8_strftime(hour_min_format(), $end_t+$resolution)."\n"
		.$row[6];
		$today[$row[0]][date($format,$am7)]["row_span"] =$row_span_count;
	}
	else
	{
		$today[$row[0]][date($format,$start_t)]["data"] = $row[3];
		$today[$row[0]][date($format,$start_t)]["long_descr"] = 
		utf8_strftime(hour_min_format(), $start_t)."~".utf8_strftime(hour_min_format(), $end_t+$resolution)."\n"
		.$row[6];
		$today[$row[0]][date($format,$start_t)]["row_span"] =$row_span_count;
	}
}
...
$sql = "select room_name, capacity, id, description from $tbl_room where area_id=$area order by sort_order";
...
else
{
	...
	echo "<tr><th width=\"1%\" nowrap>".($enable_periods ? get_vocab("period") : get_vocab("time"))."</th>";
	...
	)
	{
		...
		while (list($key, $room) = each($rooms))
		{
			...
			if(isset($today[$room][$time_t]["id"]))
			{
				...
				$row_span = $today[$room][$time_t]["row_span"];
			}
			else
				unset($id);
